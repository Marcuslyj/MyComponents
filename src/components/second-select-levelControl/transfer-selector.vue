...
<!-- 支持二次选择，支持只能选择叶节点配置 -->
  <div class="wrapper">
    <div class="ivu-transfer">
      <!-- 左侧 -->
      <div class="ivu-transfer-list" ref="left-wrapper" v-show="!readonly && !(config && config.secondSelect)">
        <div class="ivu-transfer-list-header">
          <span class="ivu-transfer-list-header-title">
            <Checkbox v-show="!searchLeftFlag" v-model="checkAll_left" @on-change="toggleAll($event,'dataLeft','checkedLeft',$refs.leftList,'checkAll_left')" :disabled="dataLeft.length ==0">未选</Checkbox>
            <Checkbox v-show="searchLeftFlag" v-model="checkAll_left_match" @on-change="toggleAll($event,'dataLeft_match','checkedLeft_match',$refs.leftList_match,'checkAll_left_match')" :disabled="dataLeft_match.length==0">未选</Checkbox>
          </span>
        </div>
        <div class="filter" v-show="showSelectType">
          <Select class="sourceSelector" v-model="onlyLeaf" size="small" style="width:118px" disabled>
            <Option value="0">自动追加同级</Option>
            <Option value="1">不自动追加同级</Option>
          </Select>
        </div>
        ...
        <div class="ivu-transfer-list-body">
          <Tree ref="leftList" v-show="!searchLeftFlag" :data="dataLeft" show-checkbox class="ivu-transfer-list-content treeNode" @on-check-change="checkChange('checkedLeft',$refs.leftList,'checkAll_left')"></Tree>
          <Tree ref="leftList_match" v-show="searchLeftFlag" :data="dataLeft_match" show-checkbox class="ivu-transfer-list-content treeNode" @on-check-change="checkChange('checkedLeft_match',$refs.leftList_match,'checkAll_left_match')"></Tree>
        </div>
      </div>
      <!-- 中间 -->
      <div class="ivu-transfer-operation" v-show="!readonly && !(config && config.secondSelect)">
        <Button size="small" type="primary" :disabled="disableAdd()" @click="add">添加</Button>
        <Button size="small" type="ghost" :disabled="disableRemove()" @click="remove">删除</Button>
      </div>
      <!-- 右侧 -->
      <div class="ivu-transfer-list" ref="right-wrapper">
        <div class="ivu-transfer-list-header">
          <span class="ivu-transfer-list-header-title" v-show="!readonly">
            <Checkbox v-show="!searchRightFlag" v-model="checkAll_right" @on-change="toggleAll($event,'dataRight','checkedRight',$refs.rightList,'checkAll_right')" :disabled="dataRight.length ==0">{{config && config.secondSelect ? "未选" :"已选"}}</Checkbox>
            <Checkbox v-show="searchRightFlag" v-model="checkAll_right_match" @on-change="toggleAll($event,'dataRight_match','checkedRight_match',$refs.rightList_match,'checkAll_left_match')" :disabled="dataRight_match.length ==0">{{config && config.secondSelect ? "未选" :"已选"}}</Checkbox>
          </span>
          ...
          <div class="filter" v-show="config && config.secondSelect && showSelectType">
            <Select class="sourceSelector" v-model="onlyLeaf" size="small" style="width:118px" disabled>
              <Option value="0">自动追加同级</Option>
              <Option value="1">不自动追加同级</Option>
            </Select>
        ...
        <div class="ivu-transfer-list-body">
          <Tree ref="rightList" v-show="!searchRightFlag" :data="dataRight" :show-checkbox="!readonly" class="ivu-transfer-list-content treeNode" @on-check-change="checkChange('checkedRight',$refs.rightList,'checkAll_right')"></Tree>
          <Tree ref="rightList_match" v-show="searchRightFlag" :data="dataRight_match" :show-checkbox="!readonly" class="ivu-transfer-list-content treeNode" @on-check-change="checkChange('checkedRight_match',$refs.rightList_match,'checkAll_right_match')"></Tree>
        </div>
      </div>
      <!-- 二次选择 -->
      <div ref="second-select-wrapper" class="second-select-wrapper" v-if="config && config.secondSelect">
        <!-- 中间 -->
        ...
        <div class="ivu-transfer-list" >
          <div class="ivu-transfer-list-header">
            <span class="ivu-transfer-list-header-title">
              <Checkbox v-show="!search_2th" v-model="checkAll_2th" @on-change="toggleAll($event,'data_2th','checked_2th',$refs.list_2th,'checkAll_2th')" :disabled="data_2th.length ==0">已选</Checkbox>
              <Checkbox v-show="search_2th" v-model="checkAll_2th_match" @on-change="toggleAll($event,'data_2th_match','checked_2th_match',$refs.list_2th_match,'checkAll_2th_match')" :disabled="data_2th.length ==0">已选</Checkbox>
            </span>
          ...
          <div class="ivu-transfer-list-body">
            <Tree ref="list_2th" v-show="!search_2th" :data="data_2th" class="ivu-transfer-list-content treeNode" show-checkbox @on-check-change="checkChange('checked_2th',$refs.list_2th,'checkAll_2th')"></Tree>
            <Tree ref="list_2th_match" v-show="search_2th" :data="data_2th_match" class="ivu-transfer-list-content treeNode" show-checkbox @on-check-change="checkChange('checked_2th_match',$refs.list_2th_match,'checkAll_2th_match')"></Tree>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      showSelectType: (this.config && this.config.showSelectType) ? true : false,
      onlyLeaf: (this.config && this.config.onlyLeaf) ? '1' : '0',


      readonly:this.config && this.config.readonly,
      title:this.config && this.config.title,
      watchFlag : true,
      dataLeft: [],
      dataRight: [],
      data_2th:[],
  ...
  created() {
    ...
    const unWatch = this.$watch(
      "data",
      (newVal, oldVal) => {
        if ((!oldVal || !oldVal.datas) && newVal && newVal.datas && this.watchFlag) {          
  ...
  methods: {
    //添加按钮的disabled状态
    disableAdd(){
      if(this.keyword_left.trim().length == 0 && this.checkedLeft.length>0){
        return false;
      }else if(this.keyword_left.trim().length > 0 && this.checkedLeft_match.length>0){
        return false;
      }
      return true;
    },
    disableAdd_2th(){
      if(this.keyword_right.trim().length == 0 && this.checkedRight.length>0){
        return false;
      }else if(this.keyword_right.trim().length > 0 && this.checkedRight_match.length>0){
        return false;
      }
      return true;
    },
    ...
    disableRemove_2th(){
      if(this.keyword_2th.trim().length == 0 && this.checked_2th.length>0){
        return false;
      }else if(this.keyword_2th.trim().length > 0 && this.checked_2th_match.length>0){
        return false;
      }
      return true;
    },
